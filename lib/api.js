...
/**
 * Check that a public id and a write token are accepted by the API.
 *
 * Sends an authenticated sync with no records, which writes nothing and
 * fails when the credentials are wrong.
 *
 * @param {string} apiUrl - Base URL of the API
 * @param {string} publicId - User's public ID
 * @param {string} writeToken - User's write token
 ...
 */
export async function verifyCredentials(apiUrl, publicId, writeToken) {
  let response;

  try {
    response = await fetch(`${apiUrl}/api/sync`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${writeToken}`,
      },
      body: JSON.stringify({ publicId, records: [] }),
    });
  } catch (err) {
