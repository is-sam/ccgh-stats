 ...
 * @param {boolean} [options.tombstones] - Pass false for an account that has
 *   just been created and so has nothing stale on the server
 * @returns {Promise<boolean>} True when the data on the server is up to date
 */
export async function runSync(options = {}) {
  const { full = false, force = false, verbose = false, tombstones = true } = options;
  ...
  try {
    ...
    const payload = buildSyncPayload(readCache(), { full, tombstones });
